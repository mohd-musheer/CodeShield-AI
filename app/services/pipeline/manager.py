...
class PipelineManager:
    ...
    def run_pipeline(
        ...
        # If a repository_url is actually a local path, treat it as custom_path
        if repository_url and Path(repository_url).exists():
            custom_path = repository_url
            repository_url = None
        ...
        try:
            ...
            try:
                ...
                # Memory overload guard: if repository is too large, abort gracefully
                if hasattr(metadata, 'total_size') and metadata.total_size > 200 * 1024 * 1024:
                    raise MemoryError('Memory overload: repository size exceeds 200MB limit.')
                # Optionally, guard against too many files
                if hasattr(metadata, 'files') and len(metadata.files) > 5000:
                    raise MemoryError('Memory overload: too many files in repository.')

                # Continue with loading files into cache as before
                file_contents = {}
                ...
                slog.error(f"Error loading repository metadata: {e}")
                raise
            ...
            for f in metadata.files:
                # Secret scanner: use scan_file if present, else fallback to generic scan
                if hasattr(self.secret_scanner, "scan_file"):
                    findings.extend(self.secret_scanner.scan_file(f))
                else:
                    findings.extend(self.secret_scanner.scan(f))
                # Config scanner: similar fallback
                if hasattr(self.config_scanner, "scan_file"):
                    findings.extend(self.config_scanner.scan_file(f))
                else:
                    findings.extend(self.config_scanner.scan(f))

            # Dependency scanner: use scan_metadata if present, else fallback to generic scan
            if hasattr(self.dependency_scanner, "scan_metadata"):
                findings.extend(self.dependency_scanner.scan_metadata(metadata))
            else:
                findings.extend(self.dependency_scanner.scan(metadata))
            ...
            for file_path, chunks in file_chunks.items():
                ...
                current_len = 0
                for c in chunks:
                    # If adding this chunk would exceed limit, merge the existing group first (if any)
                    if current_len + len(c.content) > 15000:
                        if current_group:
                            merged = self._create_merged_chunk(current_group, file_path)
                            merged_chunks.append(merged)
                        # Start a new group with the current chunk
                        current_group = [c]
                        current_len = len(c.content)
                    else:
                        current_group.append(c)
                        current_len += len(c.content)
                # After processing all chunks, merge any remaining group
                if current_group:
                    merged = self._create_merged_chunk(current_group, file_path)
                    merged_chunks.append(merged)
    ...
    def _create_merged_chunk(self, chunks: List[RepositoryChunk], file_path: str) -> RepositoryChunk:
        ...
        Returns None if the chunk list is empty.
        """
        if not chunks:
            # No chunks to merge; return a placeholder empty chunk or raise
            slog.warning(f"_create_merged_chunk called with empty chunk list for {file_path}")
            return None
        if len(chunks) == 1:
            return chunks[0]
